...
export default function PenugasanPage() {
  ...
  const [penugasanList, setPenugasanList] =
    useState<PenugasanData[]>([]);

  const [mitraOptions, setMitraOptions] =
    useState<MitraOption[]>([]);

  const [kegiatanOptions, setKegiatanOptions] =
    useState<KegiatanOption[]>([]);

  const [limitByPeriode, setLimitByPeriode] =
    useState<Record<string, LimitHonor>>({});
  ...
  // =======================================================
  // FILTER KEGIATAN BARU
  // =======================================================

  const [kegiatanFilter, setKegiatanFilter] =
    useState<string>('Semua Kegiatan');
  ...
  const [detailPenugasan, setDetailPenugasan] =
    useState<PenugasanData | null>(null);
  ...
  const [isLimitBlockedModalOpen, setIsLimitBlockedModalOpen] =
    useState<boolean>(false);

  const [limitBlockedInfo, setLimitBlockedInfo] =
    useState<LimitBlockedInfo | null>(null);

  // =======================================================
  // MODAL DUPLIKAT
  // =======================================================

  const [isDuplicateBlockedModalOpen, setIsDuplicateBlockedModalOpen] =
    useState<boolean>(false);

  const [duplicateBlockedInfo, setDuplicateBlockedInfo] =
    useState<DuplicateBlockedInfo | null>(null);
  ...
  const fetchDropdownData = useCallback(async () => {
    try {
      ...
      const {
        ...
        .from('mitra')
        .select(
          'sobat_id, nama_mitra, posisi_mitra'
        )
        .order('nama_mitra');
      ...
      const {
        ...
        .from('kegiatan')
        .select(
          'id, nama_kegiatan, kode_kegiatan, bulan_kegiatan'
        )
        .order('nama_kegiatan');
      ...
      if (resKegiatan) {
        setKegiatanOptions(resKegiatan);
      }
      ...
      const {
        ...
        .from('limit_honor')
        .select(
          'id, tahun_bulan, batas_maksimal, persen_peringatan'
        );
  ...
  const fetchPenugasan = useCallback(async () => {
    ...
    try {
      ...
      let filteredData: PenugasanData[] =
        data || [];
  ...
  // =======================================================
  // AKUMULASI HONOR MITRA + PERIODE
  // =======================================================

  const accumulatedHonorBySobatPeriode =
    useMemo(() => {
      const map: Record<
        string,
        number
  ...
  // =======================================================
  // AKUMULASI PENCAIRAN MITRA + PERIODE
  // =======================================================

  const accumulatedDicairkanBySobatPeriode =
    useMemo(() => {
      const map: Record<
        string,
        number
  ...
  const checkMitraAlreadyAtLimit =
    useCallback(
      (
        sobatId: string,
        periode: string,
        excludePenugasanId?: number
      ): LimitBlockedInfo | null => {
        if (
          !sobatId ||
          !periode
        ) {
  ...
  // =======================================================
  // CEK DUPLIKAT KEGIATAN
  // =======================================================

  const checkDuplicateAssignment =
    useCallback(
      (
        sobatId: string,
        kegiatanId: number,
  ...
  const handleSelectMitraInForm =
    (sobatId: string) => {
      const blockedLimit =
        checkMitraAlreadyAtLimit(
          sobatId,
          currentFormPeriode,
          isEditMode
            ? formData.id
      ...
      if (blockedDuplicate) {
        ...
        setIsDuplicateBlockedModalOpen(
          true
        );
  ...
  const handleSelectKegiatanInForm =
    (kegiatanId: number) => {
      const kegiatanTerpilih =
        kegiatanOptions.find(
          (k) =>
            k.id ===
            kegiatanId
  ...
  const handleBulkDelete =
    async () => {
      ...
      try {
        ...
        alert(
          'Gagal menghapus penugasan terpilih: ' +
            (
              error.message ||
              'Terjadi kesalahan'
            )
  ...
  const handleBulkStatusChange =
    async () => {
      ...
      try {
        ...
        alert(
          `Status ${selectedIds.length} penugasan berhasil diubah menjadi "${bulkStatusValue}".`
        );
  ...
  const handleExportPDF =
    () => {
      if (
        ...
        alert(
          'Tidak ada data penugasan untuk diexport.'
        );
      ...
      doc.text(
        'Daftar Penugasan Mitra Statistik',
        14,
        21
      );
      ...
      doc.save(
        `Data_Penugasan_BPS_Mojokerto_${new Date()
          .toISOString()
          .slice(
            0,
            10
  ...
  const handleOpenEditModal =
    (
      ...
      setFormData({
        ...
        kegiatan_id:
          penugasan.kegiatan_id,

        total_honor:
          penugasan.total_honor ||
          0,

        jumlah_dicairkan:
          penugasan.jumlah_dicairkan ||
          0,
  ...
  const handleSavePenugasan =
    async (
      ...
      if (
        ...
        alert(
          'Pilih Mitra dan Kegiatan BPS terlebih dahulu!'
        );
      ...
      const honorAllocated =
        Number(
          formData.total_honor
        ) || 0;

      const honorDisbursed =
        Number(
          formData.jumlah_dicairkan
        ) || 0;
      ...
      if (
        ...
        alert(
          `Gagal Menyimpan! Jumlah yang dicairkan (Rp ${honorDisbursed.toLocaleString(
            'id-ID'
          )}) tidak boleh melebihi Hak Honor Alokasi (Rp ${honorAllocated.toLocaleString(
            'id-ID'
          )}).`
        );
      ...
      if (
        ...
        const confirmExceed =
          window.confirm(
            `PERINGATAN: Total honor akumulasi mitra ini di periode ${currentFormPeriode} (Rp ${formLimitCheck.newTotal.toLocaleString(
              'id-ID'
            )}) melebihi batas limit (Rp ${formLimitCheck.maxLimit.toLocaleString(
              'id-ID'
      ...
      try {
        ...
        // ---------------------------------------------
          ...
          alert(
            'Penugasan baru berhasil ditambahkan.'
          );
        }
  ...
  const handleDeletePenugasan =
    async (
      ...
      if (
        !window.confirm(
          `Apakah Anda yakin ingin menghapus penugasan untuk "${namaMitra}"?`
        )
      ) {
        return;
      }
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="py-2.5 px-3.5 text-center">
                                ...
                                <span
                                  className={`px-2 py-0.5 rounded text-[10px] font-medium border ${statusStyle}`}
                                >
                                  {
                                    statusLimitLabel
                                  }
                        ...
                        <div>
                          ...
                          <p className="text-[11px] mt-0.5 text-slate-600">
                            ...
                            {isFullyPaid
                              ...
                              : `Sisa honor kegiatan yang belum dicairkan: ${formatRupiah(
                                  sisaHonorKegiatan
                                )}`}
              ...
              <div className="p-5 space-y-3 text-xs">
                ...
                <div className="flex justify-between border-b pb-2">
                  ...
                  <span className="font-semibold text-emerald-600">
                    {formatRupiah(
                      detailPenugasan.jumlah_dicairkan ||
                        0
                    )}
                  </span>
                </div>
                ...
                <div className="flex justify-between border-b pb-2">
                  ...
                  <span className="font-medium text-slate-800">
                    {
                      detailPenugasan.status_penugasan ||
                      'Ditugaskan'
                    }
                  </span>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
              ...
              <div className="p-6 flex flex-col items-center text-center">
                ...
                <p className="text-sm text-slate-500 mb-5 leading-relaxed">
                  Mitra tidak dapat ditugaskan pada kegiatan baru di periode{' '}
                  <strong>
                    {
                      limitBlockedInfo.periode
                    }
                  </strong>
                  .
                </p>
                ...
                <div className="w-full space-y-2.5 text-left text-sm mb-6">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-slate-800">
                      {formatRupiah(
                        limitBlockedInfo.hakHonorAlokasi
                      )}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-slate-800">
                      {formatRupiah(
                        limitBlockedInfo.sudahDicairkan
                      )}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-slate-800">
                      ...
                      {formatRupiah(
                        limitBlockedInfo.sisaLimit
                      )}{' '}

                      <span className="text-rose-600">
                        (
                        {
                          limitBlockedInfo.persenTerpakai
                        }
                        %)
                      </span>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
              ...
              <div className="p-6 flex flex-col items-center text-center">
                ...
                <h3 className="text-lg font-bold text-amber-600 mb-1.5">
                ...
                <p className="text-sm text-slate-500 mb-6 leading-relaxed">
                  ...
                  <strong className="text-slate-700">
                    {
                      duplicateBlockedInfo.namaMitra
                    }
                  </strong>{' '}

                  sudah memiliki penugasan pada kegiatan{' '}

                  <strong className="text-slate-700">
                    {
                      duplicateBlockedInfo.namaKegiatan
                    }
                  </strong>
                  .
